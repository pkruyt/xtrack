...
class CWSPS(BeamElement):
    ...
    _xofields={
               ...
               'ion_excitation_g2':     xo.Float64,
               'ion_excited_lifetime':  xo.Float64,
               'Map_of_Excitation':         xo.Float64[int(2000*2000)],
               'N_K1_values':     xo.Int64,
               'N_Delta_values': xo.Int64,
               'K1_max':          xo.Float64,
               'Delta_max':      xo.Float64,
               'Delta_min':      xo.Float64,
              }
    
    
    _extra_c_sources = [
            _pkg_root.joinpath('headers/constants.h'),
            _pkg_root.joinpath('beam_elements/elements_src/cwlaserSPS.h')]
    ...
    def __init__(self,  laser_direction_nx =  0,
        ...
        # Map of Excitation:
        fname = _pkg_root.joinpath('beam_elements/elements_src/IonLaserIP_data/CWMapSPS.json')
        with open(fname, 'r') as f:
            map_data = json.load(f)
            self.Excitation = np.array(map_data['Excitation probability'])
            self.N_K1_values, self.N_Delta_values = np.shape(self.Excitation)
            self.Delta_max = max(map_data['delta_range'])
            self.Delta_min = min(map_data['delta_range'])
