...
class ChirpedLaser(BeamElement):

    # Map of Excitation:
    fname = _pkg_root.joinpath('beam_elements/elements_src/laser_excitation_maps/chirped_excitation_map.json')
    with open(fname, 'r') as f:
        map_data = json.load(f)
        excitation_map = np.array(map_data['Excitation'])
    ...
    _xofields={
            ...
            'N_Omega_star_values':    xo.Int64,
            'N_Detuning_star_values': xo.Int64,
    ...
    _extra_c_sources = [
            _pkg_root.joinpath('headers/constants.h'),
            _pkg_root.joinpath('beam_elements/elements_src/chirped_laser.h')]
    ...
    def __init__(self,  laser_direction_nx =  0,
        ...
        with open(fname, 'r') as f:
            map_data = json.load(f)
            self.Excitation = np.array(map_data['Excitation'])
            self.N_Omega_star_values, self.N_Detuning_star_values = np.shape(self.Excitation)
            self.Omega_star_max = map_data['Omega_star_max']
            self.Detuning_star_max  = map_data['Detuning_star_max']
