...
/*gpufun*/
void CWSPS_track_local_particle(CWSPSData el, LocalParticle* part0){

    //The algorithm is partially from https://anaconda.org/petrenko/psi_beam_vs_laser

    double nx  = CWSPSData_get_laser_direction_nx(el);
    double ny  = CWSPSData_get_laser_direction_ny(el);
    double nz  = CWSPSData_get_laser_direction_nz(el);
    
    double laser_x = CWSPSData_get_laser_x(el);
    double laser_y = CWSPSData_get_laser_y(el);
    double laser_z = CWSPSData_get_laser_z(el);
    double w0 = CWSPSData_get_laser_waist_radius(el);

    double laser_intensity = CWSPSData_get_laser_intensity(el);
    double laser_wavelength = CWSPSData_get_laser_wavelength(el); // Hz
    
    double ion_excited_lifetime = CWSPSData_get_ion_excited_lifetime(el); // sec
    double ion_excitation_energy = CWSPSData_get_ion_excitation_energy(el); // eV
       
    // constants for the Map_of_Excitation vs OmegaRabi and Detuning:
    int64_t N_K1_values = CWSPSData_get_N_K1_values(el);
    int64_t N_Delta_values = CWSPSData_get_N_Delta_values(el);
    double  K1_max = CWSPSData_get_K1_max(el);
    double  Delta_max = CWSPSData_get_Delta_max(el);
    double  Delta_min = CWSPSData_get_Delta_min(el);
    double  dK1 = K1_max/(N_K1_values-1.0);
    double  dDelta = N_Delta_values/(Delta_max-1.0);
    
    
    // printf("OmegaRabiTau_max=%e m\n",OmegaRabiTau_max);
        ...
        if (r2 < POW2(w0))
            ...
            if (state > 0)
            {
                if (Delta_min < delta && delta < Delta_max)
                    {
                    // N_K1_values  N_Delta_Gamma_ratio_values
                    ...
                    // k1=K1_max;
                    // }
                    int64_t row = (int)floor(k1/dK1);
                    int64_t col = (int)floor(delta/dDelta);
                    int64_t idx = row*N_Delta_values + col;
                    

                    //double excitation_probability = CWSPSData_get_Map_of_Excitation(el, col,row);
                    double excitation_probability = CWSPSData_get_Map_of_Excitation(el, idx);
